...
public class CreateTaskTest {
	WebDriver driver;
	 ...
	 public void VerfyTaskCreated(){
		 try{
				...
				Thread.sleep(500);
				 ...
				 Thread.sleep(500);
				 Assert.assertTrue(UsfnPageManager.CreateGoalPage().Operations().IsTaskCreated(task, task) ,"Task is not created");
				 String timelineDateFormat = new SimpleDateFormat("MM/dd/yyyy").format(date);
				 String Timeline = timelineDateFormat+" - " +timelineDateFormat;
				 Assert.assertTrue(UsfnPageManager.CreateGoalPage().Operations().IsTaskCreated(task, Timeline) ,"TimeLine is not correct");
				 UsfnPageManager.CreateGoalPage().Operations().ClickTaskDeleteIcon(task);
				 
				 UsfnPageManager.CreateGoalPage().Elements().ConfirmationDialoogOKButton().click();
				 UsfnPageManager.CreateCpsPage().Operations().ApplyWait(driver);
				 Thread.sleep(1500);
				 Assert.assertFalse(UsfnPageManager.CreateGoalPage().Operations().IsTaskCreated(task, task) ,"Task is not deleted");
	 ...
	 @Test
	 public void CreateTaskWithoutGoalSave(){
